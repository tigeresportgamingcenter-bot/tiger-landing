import { Cpu, MemoryStick, Monitor, Video } from "lucide-react";
          ...
          {tiers.map((tier, index) => (
            <article key={tier.id} className={`rounded-2xl border p-6 sm:p-8 ${index === 0 ? "border-tiger-orange/50 bg-gradient-to-br from-tiger-red/15 to-white/[0.03] shadow-glow" : "border-white/10 bg-white/[0.03]"}`}>
              ...
              <dl className="mt-7 grid gap-3 sm:grid-cols-2">
                {specs.map(({ key, icon: Icon, label }) => <div key={key} className="flex items-start gap-3 rounded-xl bg-black/40 p-3.5"><Icon className="mt-0.5 size-4 shrink-0 text-tiger-orange" /><div><dt className="text-[10px] font-bold uppercase tracking-widest text-zinc-600">{label}</dt><dd className="mt-1 text-sm font-semibold text-zinc-200">{tier[key]}</dd></div></div>)}
              </dl>
            </article>
          ))}
        </div>

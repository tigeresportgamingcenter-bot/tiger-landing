...
function safeTextList(value: unknown, maxItems = 6): string[] {
  if (!Array.isArray(value)) return [];
  return value.filter((item): item is string => typeof item === "string" && Boolean(item.trim())).map((item) => item.trim()).slice(0, maxItems);
}
...
function mapPromotion(row: Record<string, unknown>): Promotion | null {
  ...
  return {
    ...
    image: safeImage(row.image_url, `Ảnh khuyến mãi ${row.name}`),
...
export async function getSupabaseContent(): Promise<SupabaseContent | null> {
  ...
  const pcTiers: PcTier[] = (pcTierResult.error ? [] : pcTierResult.data ?? []).map((row) => ({
    id: row.slug,
    name: row.name,
  ...
  return {
    ...
    upcomingPromotions: upcomingPromotions.length ? upcomingPromotions : undefined,

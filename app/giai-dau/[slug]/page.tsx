...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const description = tournament.summaryContent || tournament.description || `${tournament.game} tại ${tournament.branchName ?? "Tiger Esports"}`;
...
export default async function TournamentDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const lead = tournament.summaryContent || tournament.description;
      ...
      <section className="relative overflow-hidden border-b border-white/10 bg-zinc-950">
        ...
        <Container className="relative py-10 sm:py-16 lg:py-24">
          ...
          <div className="mt-10 max-w-4xl">
            ...
            {lead ? <p className="mt-6 max-w-3xl whitespace-pre-line text-lg leading-8 text-zinc-300">{lead}</p> : null}
      ...
      <Container className="py-10 sm:py-14">
        <article className="grid gap-8 lg:grid-cols-[0.72fr_1.28fr]">
          ...
          <div className="space-y-10">
            ...
            {isCompleted && tournament.summaryContent ? <section><h2 className="text-2xl font-extrabold text-white">Tổng kết giải đấu</h2><p className="mt-4 whitespace-pre-line text-base leading-8 text-zinc-400">{tournament.summaryContent}</p></section> : null}
            {!isCompleted && tournament.description ? <section><h2 className="text-2xl font-extrabold text-white">Giới thiệu giải đấu</h2><p className="mt-4 whitespace-pre-line text-base leading-8 text-zinc-400">{tournament.description}</p></section> : null}
            {tournament.rules ? <section><h2 className="text-2xl font-extrabold text-white">Thể lệ</h2><p className="mt-4 whitespace-pre-line text-base leading-8 text-zinc-400">{tournament.rules}</p></section> : null}
